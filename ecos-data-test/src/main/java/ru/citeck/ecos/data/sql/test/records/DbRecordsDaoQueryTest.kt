...
class DbRecordsDaoQueryTest : DbRecordsTestBase() {
    ...
    private fun testForDateAtt(attName: String, withCreate: Boolean) {
        ...
        val time0100 = "2021-01-01T00:00:00Z"
        ...
        addRecByTime(time0100, recWithDate0)
        assertThat(records.getAtt(recWithDate0, attName).asText()).isEqualTo(fixTime(time0100))

        val time0110 = "2021-01-01T10:00:10Z"
        ...
        addRecByTime(time0110, recWithDate1)
        assertThat(records.getAtt(recWithDate1, attName).asText()).isEqualTo(fixTime(time0110))

        val time0200 = "2021-01-02T00:00:00Z"
        ...
        addRecByTime(time0200, recWithDate2)
        assertThat(records.getAtt(recWithDate2, attName).asText()).isEqualTo(fixTime(time0200))

        listOf(time0100, time0110, time0200).forEach { time ->
        ...
        val result8 = records.query(
            baseQuery.copy {
                withQuery(Predicates.gt(attName, Instant.parse(time0110)))
            }
        )
        if (withTime) {
        ...
        val result9 = records.query(
            baseQuery.copy {
                withQuery(Predicates.ge(attName, Instant.parse(time0110)))
            }
        )
        if (withTime) {
        ...
        val result10 = records.query(
            baseQuery.copy {
                withQuery(Predicates.ge(attName, Instant.parse(time0100)))
            }
        )
