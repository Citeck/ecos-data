...
class DbRecordsMandatoryAttsTest : DbRecordsTestBase() {
    ...
    fun textAndAssocTest() {
        // A failed `updateRecord(mandatoryAssoc to null)` (mandatory violation) must leave the
        // existing assocs intact - i.e. the failed mutation's assoc removals must be rolled back.
        assumeTransactionRollbackSupported()
        registerAtts(
            listOf(
                AttributeDef.create {
                    withId("notMandatory")
