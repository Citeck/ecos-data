...
class DbColumnMigrationHandlerTest : DbRecordsTestBase() {
    ...
    fun aJsonNumberIsStoredAsTheInPlaceCastWouldStoreItTest() {
        ...
        assertThat(whatTheColumnHolds)
            .describedAs(
                "the row-by-row path and the in-place cast have to agree value for value (spec " +
                    "5.3), so the column must hold exactly what the same document cast to jsonb " +
        ...
        assertThat(records.getAtt(rec, "someAtt[]?id").asStrList())
            .describedAs(
                "ten is exactly the count at which the read path stops trusting the column and asks " +
                    "ed_associations, so a rule-11 transfer that never filled it reads back empty"
            )
    ...
    /**
     * `__child` on a link a rule-11 arrival creates comes from the **attribute definition**, and
    ...
    /**
     * The other end of it: an attribute leaving the association group takes its links out
     * of `ed_associations`, and the `_parent`/`_parentAtt` of the children those links named have to
     * go with them.
     *
     * **This is a deletability test and not a column-comparison test on purpose**, because a
     * disagreement between the two tables is only worth a round of work if an ordinary operation on
     * an ordinary record answers wrongly because of it - and this one does. `DbRecordsDeleteDao`
     * excludes child links from the "someone points at me" cleanup precisely because the
     * `notifyParent` block handles them, and that block mutates the parent's child attribute; with
     * the attribute no longer a child association, `RecMutAssocHandler.validateChildAssocs` throws
     * `'someAtt' is not a child association` and the record cannot be deleted. Not now and not ever:
     * the same throw meets every attempt, and deleting the parent does not take the child with it
     * either, because that cascade is driven from `ed_associations`, which no longer has the row.
     *
     * The type change here is exactly the feature's own scenario - a child association an
     * administrator turns into text - so this is reachable on completely ordinary data.
     */
    @Test
    fun aChildReleasedByADepartureIsStillDeletableTest() {
        ...
        // The headline. Before the departure released the child this threw
        // "'someAtt' is not a child association".
    ...
    /**
     * The same state read from the other side: the two columns are cleared **together**.
     *
     * Both or neither is not a matter of taste. `DbRecordsDeleteDao` gates the whole parent
     * notification on `parentRefId != null && parentAtt.isNotBlank()`, so a child left holding
     * `_parent` with a blank `_parentAtt` would delete perfectly well and leave the parent's link
     * behind it - the dangling reference this feature's own arrival path refuses to create.
     */
    @Test
    fun aDepartureClearsBothHalvesOfTheBackReferenceTest() {

        asChildAssoc()
        val child = createRecord("someKey" to "c")
        createRecord("someAtt" to listOf(child.toString()))

        asTextArray()
        triggerTheTransition()
        drain()

        assertThat(records.getAtt(child, "_parent?id").asText()).isEmpty()
        assertThat(records.getAtt(child, "_parentAtt").asText()).isEmpty()
    }
    ...
    fun theDepartureACancellationPerformsReleasesTheChildrenTooTest() {
        ...
        assertThat(records.getAtt(child, "_notExists?bool").asBoolean())
            .describedAs("released by the cancellation just as it would be by a drain")
            .isTrue()
    }
    ...
    fun aDepartureLeavesTheChildrenOfOtherAttributesAloneTest() {
        ...
        assertThat(records.getAtt(leaving, "_parent?id").asText())
            .describedAs("the child of the attribute that left is released")
            .isEmpty()
